...
* Background reading: [Pilla et al. (2020) Deeper waters are changing less consistently than surface waters in a global analysis of 102 lakes](https://drive.google.com/drive/folders/1opLlrmF7kfo0FtWwwJ1CbfPWIyuaISC1?usp=share_link)
...
1. Go to our [class data repository](https://drive.google.com/drive/u/1/folders/1FR2FLiW3xyD4Er52YzVTToPZuXXReGRo){target="_blank"}, and download `global_surface_anomalies.csv`. 
1. Make sure the file remains in the same simple format, "comma separated values" (.csv).
1. Put `global_surface_anomalies.csv` in your working directory, `Rwork`.
1. Open `global_surface_anomalies.csv` in a spreadsheet application. Don't save the file in the spreadsheet application. Do **not** save it in the spreadsheet application because it will try to change the format.
