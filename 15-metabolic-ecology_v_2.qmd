...
1. Confidence intervals of the allometric scaling exponent ($z$) for both N and P excretion, and for temperature sensitivity ($E_a$).
1. Address these questions:
   a. How well do the predictions of the Metabolic Theory of Ecology hold up for nutrient excretion by fish under field conditions? What specific factor or factors might cause deviations from MTE predictions in the field?
   a. Suppose a lake warms by 2°C due to climate change. Based on your $E_a$ estimate, by how much would individual excretion rates change?
   a. If warming also shifts the fish population toward smaller body sizes (as has been observed in some systems), how would that interact with the temperature effect on total nutrient flux?
