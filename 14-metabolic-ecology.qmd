...
### Temperature dependence, the Arrhenius Equation, and $Q_{10}$
...
## Our data, hypotheses and predictions
...
What are 16°C to 29°C in Farenheit degrees? (The conversion factor is $Y^\circ\, \mathrm(F) =  \frac{180}{100} X^\circ\, \mathrm{C} + 32$)
...
Here you need to estimate the allometric scaling exponent ($z$) for both N and P excretion rates. The key question is: how do you deal with the fact that excretion rates were measured at different temperatures?
...
When we regress $\log_{10}(B)$ on $\log_{10}(M)$, the slope of the line *is* the allometric exponent $z$ from @eq-allometry. This follows directly from the math:

$$\log B = \log B_0 + b \cdot \log M$$

This is in the form $y = a + bx$, so ordinary linear regression gives us $z$ as the slope. If MTE is correct, $b \approx 0.75$. If scaling is isometric, $b \approx 1.0$.
...
#### Approach 1: Ignore the rest of the other variable
...
m_N_all <- lm(log_N ~ inv.kT, data = fish)
...
m_P_all <- lm(log_P ~ log_mass + inv.kT, data = fish)

confint(m_P_all)
```

The slope is our estimate of the allometric scaling exponent $z$. Is it closer to 0.75 or to 1?
...
```{r}
#| label: fig-regression-N
#| fig-cap: "Linear regression of log N excretion on inverse temperature."
...
1. **Allometric analysis** (from @sec-assign1):
   ...
   b. Estimates of the allometric scaling exponent ($z$) for both N and P excretion, from both the pooled analysis and the per-temperature analysis.
