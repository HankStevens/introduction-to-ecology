...
1. Put `SmithMartin.csv` in your data directory, `Rwork/data`.
...
```{r}
liana <- read.csv("data/SmithMartin.csv", # the CSV file path name
...
```
And remember that you can save a graph into your `figs` folder using `ggsave()` right after you make your graph, like this.
```{r}
ggplot(data=liana, # we specify the data frame
      aes(x = Month, # x-axis
          y= MiddayMPa, # y-axis
  ...
  geom_boxplot()
ggsave("figs/myMiddayPlot.png", width=4, height=4)
```
...
Turn in two documents, one document has your ideas, graphs, and answers, and the other document is your R script.

A. The first document will contain:

1. A restatement, in your own words, of the hypothesis of Smith-Martin and her colleagues.
1. One prediction that follows from the hypothesis, which you can evaluate using the data we have (see above). You will take a picture of this hand-drawn image, and put it into this document.
1. One graph done in R that uses data to evaluate the hypothesis of Smith-Martin and her colleagues. The variables in the graphs must match the variables in your predictions.
1.	Identify the independent variable(s) and the dependent variable. Note that you may have more than one independent variable. Any variable that you condition on in your graph is functioning as an independent variable.
1.	Explain what the graph shows and what it tells us about the hypothesis. We can’t draw firm conclusions, so use terms such as "supports", "suggests", "is consistent with", "seems to show", or others, which help avoid expressing unwarranted certainty. Even with careful analysis, we always have to be cautious.

B. Your second document is your R script. The R script should contain *everything a person would need* aside from the data itself to make the figures that you turn in, as well as identifying information. For instance, this should include:

- Your name, the course, the chapter or assignment, and the date, all as comments with leading hastags (#s).
- Loading any packages you need, such as `tidyverse`, `ggplot2`, or others.
- code to load (or "read") any data you need, and comments that explain what the code does,
- code to organize the data if you need to, and comments that explain what the code does,
- code to create, and to save any graphs you make, and comments that explain what the code does.
